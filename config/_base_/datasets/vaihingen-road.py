...
from jscv.datasets.augument import SingleClass
...
rgb_dict = {class_name: Ana.白, "others": [88, 88, 88]}
classes = [class_name, "others"]

trainer_do_cutoff = False
num_classes = 2
ignore_index = 2
def train_aug(crop_size=train_crop_size, mean=train_mean, std=train_std):
    if crop_size is None:
        crop_size = ORIGIN_IMG_SIZE
    aug = train_aug_simple(ignore_index, crop_size, mean, std, max_ratio=0.75)
    aug.append(SingleClass(class_id))
    return aug

def val_aug(crop_size=val_crop_size, mean=train_mean, std=train_std):
    aug = test_aug_org(crop_size, ignore_index, mean, std)
    aug.append(SingleClass(class_id))
    return aug
