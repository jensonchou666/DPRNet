...
class RefineVitBlock(nn.Module):
    ...
    def forward(self, feature_map: torch.Tensor, uncertain_map: torch.Tensor):
        ...
        win_score_idx = win_score_idx.unsqueeze(-1).unsqueeze(-1).expand(B, nWF, winsz, C)
        win_x_filter = torch.gather(win_x, 1, win_score_idx).reshape(B*nWF, winsz, C)
        ...
        ''' compose '''
        win_x.scatter_(1, win_score_idx, win_x_filter.reshape(B, nWF, winsz, C))
...
class NormalVitBlock(nn.Module):
    ...
    def forward(self, feature_map: torch.Tensor, uncertain_map: torch.Tensor):
        ...
        win_x = x.permute(0, 2, 3, 1)
        
        count_refine.record_time(first=True)
        
        ''' local branch'''
        win_x = win_x.reshape(B, nH, wsz, nW, wsz, C).transpose(2, 3).reshape(
            B*nWin, winsz, C)
...
if __name__ == "__main__":
    ...
    if debug_block == 0:
        ...
        # 0.5       1254.1595458984375  1/1.723                 下降 1/1.58712(比normal)
        # 0.3       892.0120849609375   1/1.603    2.42<3.33    下降 1/2.2314(比normal)
        # 0.25      807.8231811523438                           下降 1/2.4640(比normal)
        # 0.15      617.8622436523438   1/1.443
        #(64)
        ...
        cfgs = dict(
            ...
            filter_rate=0.3)
        H = 128
        m = RefineVitLayer(64, 64, 4, _cfg(cfgs), refine_block_cls=RefineVitBlock,
                           use_conv_in=False, embed_conv=False).cuda()
        x = torch.randn(2, 64, H, H).cuda()
        ...
        m = RefineVitLayer(256, 128, 4, _cfg(cfgs)).cuda()

        x = torch.randn(2, 256, H, H).cuda()
        mask = torch.randn(2, H, H).cuda()
