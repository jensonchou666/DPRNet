...
class RefineVitBlock(nn.Module):
    ...
    def filter_window(self, x, uncertain_map):
        ...
        win_unc_map = uncertain_map.reshape(B, nH, wsz, nW, wsz).transpose(
            2, 3).contiguous().view(B, nWin, winsz)
        
        #1
        win_x = x.permute(0, 2, 3, 1).contiguous().view(
            B, nH, wsz, nW, wsz, C).transpose(2, 3).contiguous().view(B*nWin, winsz, C)
        #todo 2 error
    ...
    def forward(self, feature_map: torch.Tensor, uncertain_map: torch.Tensor):
        ...
        kv = self.kv_g(global_x.reshape(B, C, pz*pz).transpose(1, 2)
                       ).reshape(B, pz*pz, 2, C)
        k, v = kv[:, :, 0], kv[:, :, 1]
        del kv
...
def refine_vit_layer_args_normal(channels=128, local_repeat=3, filter_rate=0.4):
...
if __name__ == "__main__":
    ...
    if debug_block == 0:
        ...
        H=128, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 16, 'filter_rate': 1}:     4092.030517578125
        H=128, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 16, 'filter_rate': 0.25}:  1339.50439453125
        
        H=256, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 16, 'filter_rate': 1}:     15334.1552734375
        H=256, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 16, 'filter_rate': 0.25}:  4595.236328125

        
        H=256, C=64, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 1}:       6639.5859375
        H=256, C=64, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 0.5}:     3740.957275390625
        H=256, C=64, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 0.25}:    2273.833984375  (/2.919)

        H=256, C=128, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 1}:      11688.802734375
        H=256, C=128, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 0.5}:    6641.49658203125
        H=256, C=128, {'window_size': 8, 'local_repeat': 2, 'pool_size': 8, 'filter_rate': 0.25}:   4064.96508789   (/2.875)
        
        H=256, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 8, 'filter_rate': 1}:      14872.3681640625
        H=256, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 8, 'filter_rate': 0.5}:    8178.2392578125
        H=256, C=128, {'window_size': 8, 'local_repeat': 3, 'pool_size': 8, 'filter_rate': 0.25}:   4852.81640625   (/3.064)

        
        H=512, C=64, {'window_size': 8, 'local_repeat': 3, 'pool_size': 8, 'filter_rate': 0.8}: 25937.927734375
        H=512, C=64, {'window_size': 8, 'local_repeat': 3, 'pool_size': 8, 'filter_rate': 0.1}: 5910.14501953125 (/4.388)
        '''
        cfgs = dict(
            window_size=8,
            local_repeat=3,
