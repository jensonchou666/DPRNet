...
from .refiner_v2 import Refiner
...
class MattingRefine(MattingBase):
    ...
    def __init__(self,
        ...
        refiner = Refiner(**refiner_args)
        
        
        super().__init__(backbone, out_channels=2 + refiner.in_channel - in_channel)
    ...
    def forward(self, x, **kargs):
        ...
        x_2 = F.interpolate(x, scale_factor=self.backbone_scale*2,mode='bilinear')
        
        x = F.interpolate(x, scale_factor=self.backbone_scale, mode='bilinear')
        ...
        hid_sm = torch.relu(x[:, 2:])
        
        hid_sm = torch.cat([hid_sm, x_2], dim=1)
        # print(hid_sm.shape)
        ...
        if self.do_refine:
            ...
            # 1/2
            pha, ref_sm = self.refiner(pha_sm, err_sm, hid_sm)
            pha = pha.clamp_(0., 1.)
            ...
            return {'pred': pha.copy(), "coarse_pred": coarse_pha, "pred_err": err_sm, "refine_region": ref_sm}
        else:
...
class MattingRefineLoss(torch.nn.Module):
    def __init__(self, global_dict, MattingRefine_model,
                 pred_err_after_e=4, refine_after_e=15,
                 frozen_x_epochs=-1,) -> None:
        super().__init__()
        ...
        self.frozen_x_epochs = frozen_x_epochs
        self.frozened = False
    ...
    def forward(self, result, true_pha):
        ...
        if self.model.do_refine:
            ...
            if self.frozened and t > self.refine_after_e + self.frozen_x_epochs:
                for (name, param) in self.model.backbone.named_parameters():
                        param.requires_grad = True
                self.frozened = False
                print("info: self.frozened = False")
            ...
            if t >= self.refine_after_e:
                ...
                if not self.frozened and self.frozen_x_epochs > 0:
                    for (name, param) in self.model.backbone.named_parameters():
                         param.requires_grad = False
                    self.frozened = True
                    print("info: self.frozened = True")
