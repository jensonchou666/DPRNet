...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(
        ...
        nfs = self.num_features = []
        for i_layer in range(self.num_layers):
            nfs.append(int(embed_dim * 2 ** (i_layer + 1)))
            layer = BasicLayer(
        ...
        if not features_only:
            ...
            self.num_features[-1] = int(embed_dim * 2 ** (self.num_layers - 1))
    ...
    def forward(self, x):
        if self.features_only:
            fs = self.forward_features(x)
            for i in range(len(fs)):
                x = fs[i].permute(0, 2, 1)
                shape0 = x.shape
                H = int(shape0[2] ** 0.5)
                fs[i] = x.reshape(shape0[0], shape0[1], H, H)
            return fs
        else:
            fs = self.forward_features(x)
            x = self.forward_head(fs[-1])
            return x
