...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, extract_features=False, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
    ...
    def forward_features(self, x):
        ...
        x1, (H, W) = self.patch_embed_a(x)
        for i, blk in enumerate(self.blocks_a):
            x1 = blk(x1, H, W, self.relative_pos_a)
        x1 = x1.reshape(B, H, W, -1).permute(0, 3, 1, 2)

        x2, (H, W) = self.patch_embed_b(x1.contiguous())
        for i, blk in enumerate(self.blocks_b):
            x2 = blk(x2, H, W, self.relative_pos_b)
        x2 = x2.reshape(B, H, W, -1).permute(0, 3, 1, 2)
    
        x3, (H, W) = self.patch_embed_c(x2.contiguous())
        for i, blk in enumerate(self.blocks_c):
            x3 = blk(x3, H, W, self.relative_pos_c)       
        x3 = x3.reshape(B, H, W, -1).permute(0, 3, 1, 2)

        x4, (H, W) = self.patch_embed_d(x3.contiguous())
        for i, blk in enumerate(self.blocks_d):
            x4 = blk(x4, H, W, self.relative_pos_d)
        B, N, C = x4.shape
        x4 = x4.permute(0, 2, 1).reshape(B, C, H, W)
    ...
    def traverse(self, stat: StatisticModel, x):
        ...
        x = stat.step(nn.Sequential(
            self.stem_conv1,
            self.stem_relu1,
            self.stem_norm1
        ), x, name='stem1_conv+relu+norm')

        x = stat.step(nn.Sequential(
            self.stem_conv2,
            self.stem_relu2,
            self.stem_norm2
        ), x, name='stem2_conv+relu+norm')

        x = stat.step(nn.Sequential(
            self.stem_conv3,
            self.stem_relu3,
            self.stem_norm3
        ), x, name='stem3_conv+relu+norm')
  
        
        x1, (H, W) = self.patch_embed_a(x)
        for i, blk in enumerate(self.blocks_a):
            x1 = stat.step(blk, (x1, H, W, self.relative_pos_a), name=f'S1-{blk._get_name()}-{i}')
        x1 = x1.reshape(B, H, W, -1).permute(0, 3, 1, 2)

        x2, (H, W) = self.patch_embed_b(x1.contiguous())
        for i, blk in enumerate(self.blocks_b):
            x2 = stat.step(blk, (x2, H, W, self.relative_pos_b), name=f'S2-{blk._get_name()}-{i}')
        x2 = x2.reshape(B, H, W, -1).permute(0, 3, 1, 2)
    
        x3, (H, W) = self.patch_embed_c(x2.contiguous())
        for i, blk in enumerate(self.blocks_c):
            x3 = stat.step(blk, (x3, H, W, self.relative_pos_c), name=f'S3-{blk._get_name()}-{i}')
        x3 = x3.reshape(B, H, W, -1).permute(0, 3, 1, 2)

        x4, (H, W) = self.patch_embed_d(x3.contiguous())
        for i, blk in enumerate(self.blocks_d):
            x4 = stat.step(blk, (x4, H, W, self.relative_pos_d), name=f'S4-{blk._get_name()}-{i}')
        B, N, C = x4.shape
        x4 = x4.permute(0, 2, 1).reshape(B, C, H, W)
...
def _create_cmt_model(pretrained=False, distilled=False, **kwargs):
    ...
    if 'img_size' in kwargs:
        ...
        img_size = default_cfg['input_size'][-1]

    num_classes = kwargs.pop('num_classes', default_num_classes)
    img_size = kwargs.pop('img_size', img_size)
...
@register_model
def cmt_ti(pretrained=False, extract_features=False, **kwargs):
    """ 
    CMT-Tiny
    """
    model_kwargs = dict(qkv_bias=True, extract_features=extract_features, **kwargs)
    model = _create_cmt_model(pretrained=pretrained, **model_kwargs)
    return model

@register_model
def cmt_xs(pretrained=False, extract_features=False, **kwargs):
    """ 
    CMT-XS: dim x 0.9, depth x 0.8, input 192
    """
    model_kwargs = dict(
        qkv_bias=True, extract_features=extract_features, embed_dims=[52,104,208,416], stem_channel=16, num_heads=[1,2,4,8],
        depths=[3,3,12,3], mlp_ratios=[3.77,3.77,3.77,3.77], qk_ratio=1, sr_ratios=[8,4,2,1], **kwargs)
    model = _create_cmt_model(pretrained=pretrained, **model_kwargs)
    return model

@register_model
def cmt_s(pretrained=False, extract_features=False, **kwargs):
    """ 
    CMT-Small
    """
    model_kwargs = dict(
        qkv_bias=True, extract_features=extract_features, embed_dims=[64,128,256,512], stem_channel=32, num_heads=[1,2,4,8],
        depths=[3,3,16,3], mlp_ratios=[4,4,4,4], qk_ratio=1, sr_ratios=[8,4,2,1], **kwargs)
    model = _create_cmt_model(pretrained=pretrained, **model_kwargs)
    return model

@register_model
def cmt_b(pretrained=False, extract_features=False, **kwargs):
    """ 
    CMT-Base
    """
    model_kwargs = dict(
        qkv_bias=True, extract_features=extract_features, embed_dims=[76,152,304,608], stem_channel=38, num_heads=[1,2,4,8],
...
if __name__ == '__main__':
    net = cmt_s(extract_features=True, img_size=512)
