'''
    ...
    从 https://github.com/cedricgsh/ISDNet 移植
...
def ConvModule(in_channels, out_channels, k, stride, padding, conv_cfg, norm_cfg, act_cfg):
    if norm_cfg.get('type', None) == 'groupnorm':
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, k, 1, getPadding(k)),
            nn.GroupNorm(norm_cfg.get('num_groups', 8), out_channels),
            nn.ReLU() if act_cfg is not None else nn.Identity()
        )
    if act_cfg is None:
        return ConvBN(in_channels, out_channels, k, 1, stride)
    return ConvBNReLU(in_channels, out_channels, k, 1, stride)
...
class ISDHead(BaseCascadeDecodeHead):
    def __init__(self, loss_layer=None, pretrain_path="pretrain/STDCNet813M_73.91.tar", prev_channel=128, **kwargs):
        ...
        self.stdc_net = ShallowNet(in_channels=6, pretrain_model=pretrain_path)
        ...
        self.reduce = None
        self.losses = loss_layer
        
        self.prev_conv = nn.Sequential(nn.Conv2d(prev_channel, 128, 1), nn.ReLU())
    ...
    def forward(self, inputs, prev_output, train_flag=True):
        ...
        prev_output = self.prev_conv(prev_output)
...
# self.fuse16(shallow_feat16, deep_feat):   torch.Size([1, 512, 224, 224]) torch.Size([1, 128, 112, 112])
# self.fuse8(shallow_feat8, fused_feat_16):   torch.Size([1, 256, 448, 448]) torch.Size([1, 128, 224, 224])
        
        
        
        # print("self.fuse16(shallow_feat16, deep_feat):  ", shallow_feat16.shape, deep_feat.shape)
        

        _, aux_feat16, fused_feat_16 = self.fuse16(shallow_feat16, deep_feat)
        # stage 2
        
        # print("self.fuse8(shallow_feat8, fused_feat_16):  ", shallow_feat8.shape, fused_feat_16.shape)
        ...
        if train_flag:
            ...
            # print(output_aux16.shape, output_aux8.shape, deep_feat.shape)
    ...
    def forward_train(self, inputs, prev_output, gt_semantic_seg):
        ...
        # print(seg_logits.shape, seg_logits_aux16.shape, seg_logits_aux8.shape, gt_semantic_seg.shape)
        seg_logits = F.interpolate(seg_logits, gt_semantic_seg.shape[-2:], mode='bilinear')
        seg_logits_aux16 = F.interpolate(seg_logits_aux16, gt_semantic_seg.shape[-2:], mode='bilinear')
        seg_logits_aux8 = F.interpolate(seg_logits_aux8, gt_semantic_seg.shape[-2:], mode='bilinear')
