...
class GL_FFN(nn.Module):
    def __init__(self, 
        ...
        channel_ratio = 1/3
        C1 = C1_l+C1_g
        outc_1 = int(C1*channel_ratio)

        C2 = C2_l+C2_g
        outc_2 = int(C2*channel_ratio)

        C3 = C3_l+C3_g
        outc_3 = int(C3*channel_ratio)

        C4 = C4_l+C4_g
        outc_4 = int(C4*channel_ratio)

        if blocks > 0:
            self.layer1 = []
            for k in range(blocks):
                inc = C1 if k==0 else outc_1
                self.layer1.append(ConvBNReLU(inc, outc_1, 3))
            self.layer1 = ResBlocks(*self.layer1)

            self.layer2 = []
            for k in range(blocks):
                inc = C2 if k==0 else outc_2
                self.layer2.append(ConvBNReLU(inc, outc_2, 3))
            self.layer2 = ResBlocks(*self.layer2)

            self.layer3 = []
            for k in range(blocks):
                inc = C3 if k==0 else outc_3
                self.layer3.append(ConvBNReLU(inc, outc_3, 3))
            self.layer3 = ResBlocks(*self.layer3)

            self.layer4 = []
            for k in range(blocks):
                inc = C4 if k==0 else outc_4
                self.layer4.append(ConvBNReLU(inc, outc_4, 3))
        ...
        self.pose_decoder=pose_decoder_cls(
            [C0_l, outc_1, outc_2, outc_3, outc_4],
            **decoder_args
        )
    ...
    def forward(self, g_fs, l_fs):
        f0, f1, f2, f3, f4 = l_fs
        g1, g2, g3, g4 = g_fs
        # print(f1.shape, g1.shape)
        # print(f2.shape, g2.shape)
        # print(f3.shape, g3.shape)
        # print(f4.shape, g4.shape)

        f1 = torch.concat([f1, F.interpolate(g1, f1.shape[-2:], mode="bilinear")], dim=1)
        f2 = torch.concat([f2, F.interpolate(g2, f2.shape[-2:], mode="bilinear")], dim=1)
        f3 = torch.concat([f3, F.interpolate(g3, f3.shape[-2:], mode="bilinear")], dim=1)
        f4 = torch.concat([f4, F.interpolate(g4, f4.shape[-2:], mode="bilinear")], dim=1)
...
class DPRNet(nn.Module):
    def __init__(
            ...
            pred_easy_bondary_training=0.05,
            labal_err_bondary=2,    # <%
        ...
        self.global_decoder.save_features = False
    ...
    def forward_train(self, batch:dict):
        ...
        for idx, (imgij, maskij) in enumerate(zip(img_patches, mask_patches)):
            ...
            gb1, gb2, gb3, gb4 = g_fs[1], g_fs[2], g_fs[3], g_fs[4]
            gb4 = F.interpolate(gb4, scale_factor=2, mode="bilinear")
            ...
            if self.stage == 1:
                ...
                if self.stage == 3:
                    ...
                    B, C32, H32, W32 = gb2.shape
                    gb2 = gb2.detach().reshape(B, C32, SH, H32//SH, SW, W32//SW)
                    B, C32, H32, W32 = gb3.shape
                    gb3 = gb3.detach().reshape(B, C32, SH, H32//SH, SW, W32//SW)
                    B, C32, H32, W32 = gb4.shape
                    gb4 = gb4.detach().reshape(B, C32, SH, H32//SH, SW, W32//SW)
                    ...
                    for hard_idx_1 in hard_idx:
                        img_L, mask_L = [], []
                        gb1_L, gb2_L, gb3_L, gb4_L = [], [], [], []
                        for j in hard_idx_1:
                            p, q = j//SW, j%SW
                            img_L.append(imgij[:,:,p,:,q])
                            mask_L.append(maskij_reshape[:,p,q])
                            gb1_L.append(gb1[:,:,p,:,q])
                            gb2_L.append(gb2[:,:,p,:,q])
                            gb3_L.append(gb3[:,:,p,:,q])
                            gb4_L.append(gb4[:,:,p,:,q])

                        img_L = torch.concat(img_L, dim=0)
                        mask_L = torch.concat(mask_L, dim=0)
                        gb1_L = torch.concat(gb1_L, dim=0)
                        gb2_L = torch.concat(gb2_L, dim=0)
                        gb3_L = torch.concat(gb3_L, dim=0)
                        gb4_L = torch.concat(gb4_L, dim=0)

                        l_fs = self.local_encoder(resize_to(img_L, self.local_downsample))
                        l_seg = self.GL_ffn_decoder((gb1_L, gb2_L, gb3_L, gb4_L), l_fs)
                        del l_fs
...
def test_predict_speed_pure():
    ...
    print(GD.channels)
    LD = GL_FFN(GB.channels, LB.channels, 2, 1/2, FPNDecoder, local_fpn_decoder_args)
