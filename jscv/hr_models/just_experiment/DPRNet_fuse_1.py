...
class GL_FFN(nn.Module):
    ...
    def forward(self, g_fs, l_fs):
        f0, f1, f2, f3, f4 = l_fs
        if g_fs.shape[2:] != f4.shape[2:]:
            print("### g_fs.shape , f4.shape", g_fs.shape , f4.shape)
            g_fs = F.interpolate(g_fs, f4.shape[2:], mode='bilinear', align_corners=False)
...
class DPRNet(nn.Module):
    ...
    def forward_train(self, batch:dict):
        ...
        for idx, (imgij, maskij) in enumerate(zip(img_patches, mask_patches)):
            ...
            gdi = self.global_decoder.features[self.fuse_GD]
            ...
            if self.stage == 1:
                ...
                if self.stage == 3:
                    ...
                    B, C32, H32, W32 = gdi.shape
                    gdi = gdi.detach().reshape(B, C32, SH, H32//SH, SW, W32//SW)
                    # B, SH, SW
...
def test_predict_speed_pure():
    ...
    LD = GL_FFN(GD.channels[fuse_GD], LB.channels, 2, 1/2, FPNDecoder, local_fpn_decoder_args)
