...
def label_smoothed_nll_loss_map(
    lprobs: torch.Tensor, target: torch.Tensor, epsilon: float, ignore_index=None, dim=-1
) -> torch.Tensor:
    if target.dim() == lprobs.dim() - 1:
        target = target.unsqueeze(dim)
    ...
    eps_i = epsilon / lprobs.size(dim)
    loss_map = (1.0 - epsilon) * nll_loss + eps_i * smooth_loss
