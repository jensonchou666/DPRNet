...
class DynamicPatchesGrouping:
    ...
    def __init__(self, 
                 ...
                 min_pathes_nums=0.35, # 补丁数量 在 ?% 以下的分配被跳过 -- 防止补丁数过少的分配 影响速度
    ...
    def scan_patches_difficulty_table(self, PDT:torch.Tensor, threshold: float):
        with torch.no_grad():
            return self._scan_patches_difficulty_table(PDT, threshold)
    ...
    def _scan_patches_difficulty_table(self, PDT:torch.Tensor, threshold: float):
        ...
        H, W = PDT.shape
        PDT = PDT.view(-1)
        mask0 = PDT > threshold * self.threshold_rate_1x1
        if mask0.any():
            #TODO 是否要 排序
    ...
    def analyze_all(self):
        return self.analyze_items(self.num_patches_counter, self.total_pathes_counter, self.batches_counter)
...
def analyze_allocated_once(allocated_groups, manager: DynamicPatchesGrouping, PDT: torch.Tensor):
    ...
    result = {'num_patches': num_patches,  #? 待细化补丁的总数 ( 以PDT的最小块为单位，即最小补丁 )
              'select_rate': round(num_patches/total_pathes, 4),    #? 比例: 待细化补丁数 / PDT总补丁数 
              'batches': batches,  #? 细化阶段 依次进行 batches 次, 越小越好
              'memory_use_rate': round(num_patches/(max_pathes_nums*batches), 4), #? 显存利用率， 越大越好
              }
    return result
...
def paint_on_image(image: torch.Tensor, 
    ...
    for i, g in enumerate(allocated_groups):
        ...
        color = torch.tensor(enhanced_color_transition(rate), device=manager.device)
        for box in boxes:
            y1, y2, x1, x2 = box[0]
